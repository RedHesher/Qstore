...
namespace Qstore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ChangeProfile(ApplicationUser model)
        {
            if (ModelState.IsValid)
            {
                var currentUser = UserManager.FindById(User.Identity.GetUserId());
                ...
                UserManager.SetEmail(currentUser.Id, model.Email);
                return RedirectToAction("Index", "Manage", new { Message = "Profile Changes Confirmed"});
            }
            else
            {
                return View("Error");
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, DateOfBirth = model.DateOfBirth/*, Orders = new List<Order>()*/ };
